...
public class BestLowPriceByMonth implements BestPriceByMonthBetweenAllStatesInterface{
    private final List<StateUSA> statesUSA;

    public BestLowPriceByMonth(List<StateUSA> statesUSA) {
        ...
        return statesUSA
                ...
                .collect(groupingBy(state -> state.getDate().getYear(),
                        groupingBy(stateDate -> stateDate.getDate().getMonth().getValue(),
                                minBy(Comparator.comparing(StateUSA::getLowQuality)))));
    }
}
